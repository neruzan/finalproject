...
function EditNews() {
  ...
  const user = JSON.parse(localStorage.getItem("finalproject-user"));
  ...
  const save = async () => {
    ...
    try {
      ...
      setLoading(false);
      toast("Notes edited successfully", "success");
      navigate("/home");
    } catch (error) {
      console.log(error);
  ...
  return (
    <Layout>
      {loading && <Spinner />}
      <h1 className="text-2xl  font-semibold mt-5 ml-5">Edit Notes</h1>
      <div className="px-5 pt-5">
        ...
        <textarea
          ...
          className="border-2  w-full border-gray-300 px-5     my-2"
          rows="4"
